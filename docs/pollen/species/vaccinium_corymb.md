# *Vaccinium corymb* (blauwe bes)

{{ gallery("vaccinium_corymb") }}

## Kenmerken

| Veld | Waarde |
|---|---|
| Latijn | Vaccinium corymb |
| Nederlands | blauwe bes |
| Grootte | 37 µm-42 µm |
| Vorm | tetrade |
| Apertuur | (3-)colporaat |
| Sculptuur | glad |
| pollen-note | Kerkvliet: glad |

### SoT (`data/pollen.yaml`)

```yaml
vaccinium_corymb:
  name:
    latin_name: Vaccinium corymb
    dutch_name: blauwe bes
  classification:
    order: null
    family_latin: null
    family_dutch: null
    tribe: null
    genus: null
  size:
    size_smallest: 37 µm
    size_largest: 42 µm
    height_px: null
  pollen_class_beug: null
  pollen_features:
    shape: tetrade
    sculpture: glad
    sculpture_visibility: null
    aperture: (3-)colporaat
    aperture_visibility: null
    ornamentation: null
    ornamentation_visibility: null
    polarity: null
    pe_ratio: null
    pollen-note: 'Kerkvliet: glad'
  flowering_time:
    start: null
    end: null
  value:
    nectar_value: null
    pollen_value: null
  note:
    note_plant: null
    note_honey: null
    note_pollen: null
  frequency_in_dutch_honey: null
  frequency_in_eu_honey: null
  frequency_in_non_eu_honey: null
  links:
    pollenX: https://pollenx.eu/species.php?species=Vaccinium_corymb
    tstebler: https://pollen.tstebler.ch/MediaWiki/index.php?title=Vaccinium_corymb
    paldat: https://www.paldat.org/pub/Vaccinium_corymb
    waarneming: https://waarneming.nl/search/?q=vaccinium+corymb
  images: []
  is_secondary_contributor: null
```

## Determinatiesleutels

### Kerkvliet
<details><summary>kerkvliet_determinatietabel</summary>

- Sectie: Pollen met stekels echinaat fenestraat

</details>

## Online databases

- pollenX: https://pollenx.eu/species.php?species=Vaccinium_corymb
- tstebler: https://pollen.tstebler.ch/MediaWiki/index.php?title=Vaccinium_corymb
- paldat: https://www.paldat.org/pub/Vaccinium_corymb
- waarneming: https://waarneming.nl/search/?q=vaccinium+corymb
