# *Silene flos-cuculi* (echte koekoeksbloem)

{{ gallery("silene_flos_cuculi") }}

## Kenmerken

| Veld | Waarde |
|---|---|
| Latijn | Silene flos-cuculi |
| Nederlands | echte koekoeksbloem |
| Familie | Caryophyllaceae (anjerfamilie) |
| Grootte | 32.9 µm-36.7 µm |
| Vorm | rond, sferoid |
| Apertuur | pantoaperturaat, periporaat, veel poren; porendiameter ca. 6 (4.6-7.3) µm |
| Sculptuur | baculaat/verrucaat tot optisch reticulaat |
| Ornamentatie | apertuurmembranen geornamenteerd, operculaat |
| pollen-note | Beug 33.8.9 Silene-type; syn. Lychnis flos-cuculi |
| Frequentie in NL-honing | rare |
| Frequentie in EU-honing | rare |
| Plantnotitie | syn. Lychnis flos-cuculi |

### SoT (`data/pollen.yaml`)

```yaml
silene_flos_cuculi:
  name:
    latin_name: Silene flos-cuculi
    dutch_name: echte koekoeksbloem
  classification:
    order: null
    family_latin: Caryophyllaceae
    family_dutch: anjerfamilie
    tribe: null
    genus: Silene
  size:
    size_smallest: 32.9 µm
    size_largest: 36.7 µm
    height_px: 92
  pollen_class_beug: Periporat
  pollen_features:
    shape: rond, sferoid
    sculpture: baculaat/verrucaat tot optisch reticulaat
    sculpture_visibility: null
    aperture: pantoaperturaat, periporaat, veel poren; porendiameter ca. 6 (4.6-7.3)
      µm
    aperture_visibility: null
    ornamentation: apertuurmembranen geornamenteerd, operculaat
    ornamentation_visibility: null
    polarity: null
    pe_ratio: null
    pollen-note: Beug 33.8.9 Silene-type; syn. Lychnis flos-cuculi
  flowering_time:
    start: null
    end: null
  value:
    nectar_value: null
    pollen_value: null
  note:
    note_plant: syn. Lychnis flos-cuculi
    note_honey: null
    note_pollen: null
  frequency_in_dutch_honey: rare
  frequency_in_eu_honey: rare
  frequency_in_non_eu_honey: null
  links:
    pollenX: https://pollenx.eu/species.php?species=Silene_flos-cuculi
    tstebler: https://pollen.tstebler.ch/MediaWiki/index.php?title=Silene_flos-cuculi
    paldat: https://www.paldat.org/pub/Silene_flos-cuculi
    waarneming: https://waarneming.nl/search/?q=silene+flos-cuculi
  images:
  - path: assets/images/by-taxon/silene_flos_cuculi/silene_flos_cuculi_1.png
    kind: by_taxon
    source: by_taxon
  - path: assets/images/by-taxon/silene_flos_cuculi/silene_flos_cuculi_2.png
    kind: by_taxon
    source: by_taxon
  - path: assets/images/by-taxon/silene_flos_cuculi/silene_flos_cuculi_3.png
    kind: by_taxon
    source: by_taxon
  is_secondary_contributor: null
```

## Determinatiesleutels

### Kerkvliet
<details><summary>kerkvliet_determinatietabel</summary>

- Sectie: Pollen met een netwerk op het oppervlak (reticulate pollen) en ander reliëf op oppervlak

</details>

## Online databases

- pollenX: https://pollenx.eu/species.php?species=Silene_flos-cuculi
- tstebler: https://pollen.tstebler.ch/MediaWiki/index.php?title=Silene_flos-cuculi
- paldat: https://www.paldat.org/pub/Silene_flos-cuculi
- waarneming: https://waarneming.nl/search/?q=silene+flos-cuculi
