# *Lupinus angustifolius* (blauwe lupine)

{{ gallery("lupinus_angustifolius") }}

## Kenmerken

| Veld | Waarde |
|---|---|
| Latijn | Lupinus angustifolius |
| Nederlands | blauwe lupine |
| Grootte | 32 µm-36 µm |
| Vorm | О ⁄ ∆ |
| Apertuur | 3-colporaat |
| Sculptuur | reticulaat |
| pollen-note | Kerkvliet: 32-36 |

### SoT (`data/pollen.yaml`)

```yaml
lupinus_angustifolius:
  name:
    latin_name: Lupinus angustifolius
    dutch_name: blauwe lupine
  classification:
    order: null
    family_latin: null
    family_dutch: null
    tribe: null
    genus: Lupinus
  size:
    size_smallest: 32 µm
    size_largest: 36 µm
    height_px: null
  pollen_class_beug: null
  pollen_features:
    shape: О ⁄ ∆
    sculpture: reticulaat
    sculpture_visibility: null
    aperture: 3-colporaat
    aperture_visibility: null
    ornamentation: null
    ornamentation_visibility: null
    polarity: null
    pe_ratio: null
    pollen-note: 'Kerkvliet: 32-36'
  flowering_time:
    start: null
    end: null
  value:
    nectar_value: null
    pollen_value: null
  note:
    note_plant: null
    note_honey: null
    note_pollen: null
  frequency_in_dutch_honey: null
  frequency_in_eu_honey: null
  frequency_in_non_eu_honey: null
  links:
    pollenX: null
    tstebler: null
    paldat: null
    waarneming: null
  images: []
  is_secondary_contributor: null
```

## Determinatiesleutels

### Kerkvliet
<details><summary>kerkvliet_determinatietabel</summary>

- Sectie: Pollen met een netwerk op het oppervlak (reticulate pollen) en ander reliëf op oppervlak

</details>
