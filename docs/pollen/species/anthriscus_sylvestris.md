# *Anthriscus sylvestris* (fluitenkruid)

{{ pollen_gallery("anthriscus_sylvestris") }}

## Kenmerken

| Veld | Waarde |
|---|---|
| Latijn | Anthriscus sylvestris |
| Nederlands | fluitenkruid |
| Familie | Apiaceae (schermbloemenfamilie) |
| Grootte | 24 µm-27 µm |
| Vorm | rond to licht driehoekig convex, prolaat |
| Polariteit | isopolair |
| Apertuur | smalle colpi |
| Sculptuur | verrucaat, scabraat, gemmaat |
| Ornamentatie | geen |
| Bloeitijd | 5-6 |
| Nectarwaarde | 2 |
| Pollenwaarde | 2 |
| pollen_class | 14 |

### SoT (`data/pollen.yaml`)

```yaml
anthriscus_sylvestris:
  latin: Anthriscus sylvestris
  dutch: fluitenkruid
  family: Apiaceae (schermbloemenfamilie)
  size:
    smallest_size: 24 µm
    largest_size: 27 µm
    height_px: 65
  pollen_class: 14
  shape: rond to licht driehoekig convex, prolaat
  sculpture: verrucaat, scabraat, gemmaat
  aperture: smalle colpi
  ornamentation: geen
  polarity: isopolair
  pe_ratio: null
  pollen-note: null
  flowering_time:
    start: 5
    end: 6
  nectar_value: 2
  pollen_value: 2
  frequency_in_dutch_honey:
  frequency_in_eu_honey:
  frequency_in_non_eu_honey:
  links:
    pollenX: https://pollenx.eu/species.php?species=Anthriscus_sylvestris
    tstebler: https://pollen.tstebler.ch/MediaWiki/index.php?title=Anthriscus_sylvestris
    paldat: https://www.paldat.org/pub/Anthriscus_sylvestris
    waarneming:
  - source: vanderham
    path: /Users/md/Developer/pollenID/docs/keys/vanderham/vanderham-pollentabel.json
  - source: kerkvliet
    path: /Users/md/Developer/pollenID/docs/keys/kerkvliet/kerkvliet-determinatietabel.json
  images:
  - path: assets/images/by-taxon/anthriscus_sylvestris/anthriscus_sylvestris_1.png
    kind: pollenwiki
    source: pollenwiki
  - path: assets/images/by-taxon/anthriscus_sylvestris/anthriscus_sylvestris_2.png
    kind: pollenwiki
    source: pollenwiki
  - path: assets/images/by-taxon/anthriscus_sylvestris/anthriscus_sylvestris_3.png
    kind: pollenwiki
    source: pollenwiki
  - path: assets/images/by-taxon/anthriscus_sylvestris/anthriscus_sylvestris_4.png
    kind: pollenwiki
    source: pollenwiki
```

## Determinatiesleutels

[to be verified]

## Online databases

- pollenX: https://pollenx.eu/species.php?species=Anthriscus_sylvestris
- tstebler: https://pollen.tstebler.ch/MediaWiki/index.php?title=Anthriscus_sylvestris
- paldat: https://www.paldat.org/pub/Anthriscus_sylvestris
