# *Hypericum perforatum* (St. Janskruid)

{{ pollen_gallery("hypericum_perforatum") }}

## Kenmerken

| Veld | Waarde |
|---|---|
| Latijn | Hypericum perforatum |
| Nederlands | St. Janskruid |
| Grootte | 20 µm-22 µm |
| Apertuur | tricolporaat |
| Sculptuur | fijn reticulaat |

### SoT (`data/pollen.yaml`)

```yaml
hypericum_perforatum:
  latin: Hypericum perforatum
  dutch: St. Janskruid
  family: null
  size:
    smallest_size: 20 µm
    largest_size: 22 µm
    height_px: 55
  pollen_class: null
  shape: null
  sculpture: fijn reticulaat
  aperture: tricolporaat
  ornamentation: null
  polarity: null
  pe_ratio: null
  pollen-note: null
  flowering_time:
    start: null
    end: null
  nectar_value: null
  pollen_value: null
  frequency_in_dutch_honey:
  frequency_in_eu_honey:
  frequency_in_non_eu_honey:
  links:
    pollenX: https://pollenx.eu/species.php?species=Hypericum_perforatum
    tstebler: https://pollen.tstebler.ch/MediaWiki/index.php?title=Hypericum_perforatum
    paldat: https://www.paldat.org/pub/Hypericum_perforatum
    waarneming:
  - source: kerkvliet
    path: /Users/md/Developer/pollenID/docs/keys/kerkvliet/kerkvliet-determinatietabel.json
  images:
  - path: assets/images/by-taxon/hypericum_perforatum/hypericum_perforatum_1.png
    kind: by_taxon
    source: by_taxon
  - path: assets/images/by-taxon/hypericum_perforatum/hypericum_perforatum_2.png
    kind: by_taxon
    source: by_taxon
  - path: assets/images/by-taxon/hypericum_perforatum/hypericum_perforatum_3.png
    kind: by_taxon
    source: by_taxon
  - path: assets/images/by-taxon/hypericum_perforatum/hypericum_perforatum_4.png
    kind: by_taxon
    source: by_taxon
  - path: assets/images/by-taxon/hypericum_perforatum/hypericum_perforatum_5.png
    kind: by_taxon
    source: by_taxon
```

## Determinatiesleutels

[to be verified]

## Online databases

- pollenX: https://pollenx.eu/species.php?species=Hypericum_perforatum
- tstebler: https://pollen.tstebler.ch/MediaWiki/index.php?title=Hypericum_perforatum
- paldat: https://www.paldat.org/pub/Hypericum_perforatum
